...
{
	//Converte le coordinate dei vertici del triangolo dalla rappresentazione omogenea canonica a quella di pixel.
	...
	// Calcola la bounding box del triangolo nei limiti del frame buffer.
	...
	for (unsigned i = lx; i <= ux; i++)
	{
		for (unsigned j = ly; j <= uy; j++)
		{
			...
			if (b.minCoeff() >= 0)
			{
				/*std::cout << " v1.normal : " << v1.normal  << std::endl;
				getchar();*/
				...
				if (va.position[2] >= -1 && va.position[2] <= 1)
				{
					...
					/*std::cout << "TRIANGLE DO NOT ENTER FRAGMENT" << std::endl;
					getchar();*/
				}
			}else{
				/*std::cout << "COEFF TRIANGLE DO NOT ENTER FRAGMENT" << std::endl;
				getchar();*/
			}
		}
	}
...
void rasterize_triangles(const Program &program, const UniformAttributes &uniform, const std::vector<VertexAttributes> &vertices, FrameBuffer &frameBuffer)
{
	...
	for (unsigned i = 0; i < vertices.size(); i++){
		/*std::cout << " vertices[i].normal : \n" << vertices[i].normal << std::endl;
		getchar();*/
		v[i] = program.VertexShader(vertices[i], uniform);
		/*std::cout << " Transformed v[i].normal : \n" << v[i].normal << std::endl;
		getchar();*/
	}
...
void rasterize_line(const Program &program, const UniformAttributes &uniform, const VertexAttributes &v1, const VertexAttributes &v2, 
float line_thickness, FrameBuffer &frameBuffer)
{
