...
Add an option to translate the object toward the camera, while rotating around
its barycenter. 
...
int main(int argc, char *argv[]) 
{
	...
	std::string basePath = "../img/ex3/Animation_";
...
Vector3f compute_center_3d_model(const MatrixXf& vertices, const MatrixXi& facets) {
    ...
    Vector3f center_of_the_model(0.0f, 0.0f, 0.0f);
    int totalVertices = 0;

    // sum of the verticies
	for (int i = 0; i < facets.rows(); ++i) {
        Vector3f v0 = vertices.row(facets(i, 0));
        Vector3f v1 = vertices.row(facets(i, 1));
        Vector3f v2 = vertices.row(facets(i, 2));

        center_of_the_model += v0 + v1 + v2;
        totalVertices += 3;
    }

    center_of_the_model /= totalVertices;

    return center_of_the_model;
}
