...
In this exercise, you will implement different shading options for the triangles.
The scene should always contain at least one light source, set as a shader uniform.
Add an option to the code to render three different versions of the model:

1. Wireframe: only the edges of the triangles are drawn.

2. Flat Shading: each triangle is rendered using a unique normal (i.e. the
normal of all the fragments that compose a triangle is simply the normal of
the plane that contains it). On top of the flat shaded triangle, you should
draw the wireframe.

3. Per-Vertex Shading: the normals are specified on the vertices of the
mesh, the color is computed for each vertex, and then interpolated in the
...
void load_off(const std::string &filename, vector<VertexAttributes> &vertex_attributes_flat,vector<VertexAttributes> &vertex_attributes_wire,vector<VertexAttributes> &vertex_attributes_per_vertex);
//Matrix4f ortographic_matrix(Vector3f top_right_corner,float far, float focal_length,float aspectRatio);
...
int main(int argc, char *argv[]) 
{
	...
	program.VertexShader = [](const VertexAttributes& va, const UniformAttributes& uniform)
	{
		...
		ModelTransformedPosition = model * va.position;
	...
	program.FragmentShader = [](const VertexAttributes& va, const UniformAttributes& uniform)
	{
		...
		if(shadingMode == FLAT_SHADING  || shadingMode == PER_VERTEX_SHADING ){
			...
			// Point In Model space need to be trasformed in Camera Space 
			...
			// Normal of the point 3d depemd on the type of shading
			if(shadingMode == PER_VERTEX_SHADING){
			...
			// Ambient light contribution , Combine Ambient Color and Material Color
			...
			// For each lights punctual lights contribution (direct lighting) 
	...
	char const * img_ex2_folder = "../img/ex2/Base_case.png";
	vector<uint8_t> image;
	framebuffer_to_uint8(frameBuffer,image);
	stbi_write_png(img_ex2_folder, frameBuffer.rows(), frameBuffer.cols(), 4, image.data(), frameBuffer.rows()*4);
...
void load_scene(const std::string &filename,Program &program,UniformAttributes &uniform, FrameBuffer &frameBuffer){
	...
	//uniform.background_color = read_vec3( scene["Background"]);
	...
	uniform.camera_matrix_inv_transpose = uniform.camera_matrix.inverse().transpose();  
	...
	for (const auto &entry : data["Lights"]) {
		...
		light.Liposition << read_vec4(entry["Position"]);
	...
	for (const auto &entry : data["Objects"]) {
		...
		if (entry["Type"] == "Mesh") {
			...
			uniform.model_matrix = create_model_matrix(
				translation, angle_x , angle_y , angle_z , scale_factors);
...
// slide 8 page 17 - 3D transformation
Matrix4f create_model_matrix(const Vector3f& translation, 
float angle_x, float angle_y , float angle_z, const Vector3f& scale_factors)
{
...
Matrix4f perspective_matrix(float near , float focal_lenght, float theta,float aspectRatio){
	...
	// TODO complete P slide 12 page 17
...
void compute_face_normals(const MatrixXf &vertices, const MatrixXi &facets, 
    ...
    for (int i = 0; i < facets.rows(); ++i) {
		...
		if (normal.dot(to_center) > 0){   // if the normal  point in the same direction of the vector from the point v0 to the center re-compute it
...
void compute_per_vertex_normals(const MatrixXf &vertices, const MatrixXi &facets, const MatrixXf &face_normals, MatrixXf &vertex_normals) {
    ...
    for (int i = 0; i < vertices.rows(); ++i) {
        if (count[i] > 0) {
			...
			vertex_normals.row(i).normalize(); //  Remember to normalize the normals after averaging. Ex.2
        }
    }
}
